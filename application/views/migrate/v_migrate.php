...
<head>
    <meta charset="utf-8" />
    <title>Database Migration Dashboard | e-Capaian</title>
...
<body class="migrate-body">
    ...
    <nav class="migrate-nav">
        <div class="container-fluid d-flex justify-content-between align-items-center">
            <a class="brand" href="<?php echo base_url('migrate'); ?>">
                <span>e-Capaian</span>&nbsp;Migration Desk
            </a>
            <div>
                <a class="nav-link" href="<?php echo base_url('welcome'); ?>">
                    <i class="mdi mdi-home-outline me-1"></i>Back to Dashboard
                </a>
            </div>
        </div>
    </nav>
    ...
    <div class="migrate-container">
        ...
        <div class="migrate-header-section">
            <div>
                <h1 class="migrate-title">Database Migration</h1>
                <p class="migrate-subtitle">Trace, execute, and rollback database schemas using the clean architecture migration system.</p>
            </div>
            <div>
                <?php if ($status->isEnabled): ?>
                    <span class="badge-clean-success">MIGRATION ACTIVE</span>
                <?php else: ?>
                    <span class="badge-clean-danger">MIGRATION DISABLED</span>
                <?php endif; ?>
            </div>
        </div>
        ...
        <?php if ($error_message): ?>
            <div class="custom-alert-banner alert-danger alert-dismissible fade show" role="alert">
                ...
                <div class="alert-content">
                    <strong>Failed:</strong> <?php echo $error_message; ?>
                </div>
                <button type="button" class="btn-close" data-bs-dismiss="alert" aria-label="Close"></button>
            </div>
        <?php endif; ?>
            ...
            <div class="col-lg-4 col-md-12 mb-3">
                <div class="diagnostic-card d-flex align-items-center">
                    ...
                    <div class="overflow-hidden">
                        <div class="label">Metadata Table & Path</div>
            ...
            <div class="d-flex flex-wrap gap-3">
                <?php if ($status->isEnabled): ?>
                    <?php echo form_open('migrate/latest', array('class' => 'm-0')); ?>
                        <button type="submit" class="btn-migrate-primary">
                            <i class="mdi mdi-arrow-up-bold-circle-outline"></i> Migrate to Latest Version
                        </button>
                    <?php echo form_close(); ?>
                ...
                <?php else: ?>
                    <div class="custom-alert-banner alert-danger m-0 w-100" role="alert">
                        ...
                        <div class="alert-content">
                            <strong>Migration is currently disabled!</strong> To execute migrations, please open <code>application/config/migration.php</code> and set <code>$config['migration_enabled'] = TRUE;</code>.
                        </div>
                    </div>
                <?php endif; ?>
            </div>
        ...
        <div class="migrate-card">
            <h5 class="migrate-card-title">
                ...
                Migration History & Schema Files
            </h5>
                ...
                <table class="table table-migrate">
                    <thead>
                        <tr>
                            ...
                            <th style="width: 25%;">Version (Timestamp)</th>
                            <th style="width: 25%;">Description</th>
                            <th style="width: 20%;">File Name</th>
                            <th style="width: 15%; text-align: right;">Control</th>
                        </tr>
                    </thead>
                    <tbody>
                                ...
                                <tr>
                                    <td>
                                        <?php if ($migration->isApplied): ?>
                                            <span class="badge-clean-success">Applied</span>
                                        <?php else: ?>
                                            <span class="badge-clean-warning">Pending</span>
                                        <?php endif; ?>
                                    </td>
                                    <td>
                                        ...
                                        <?php if ($status->isEnabled): ?>
                                            ...
                                            <?php else: ?>
                                                ...
                                                <?php echo form_open('migrate/version', array('class' => 'd-inline')); ?>
                                                    <input type="hidden" name="version" value="<?php echo $migration->version; ?>">
                                                    <button type="submit" class="btn-action-outline-success" title="Migrate up to this specific version">
                                                        Migrate To Here
                                                    </button>
                                                <?php echo form_close(); ?>
                                            <?php endif; ?>
                                        <?php else: ?>
                                            <span class="btn-action-disabled">Disabled</span>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
        ...
        <!-- Footer -->
        <footer class="text-center text-muted py-5" style="border-top: 1px solid #f1f5f9; margin-top: 3rem;">
            <p class="mb-1" style="font-size: 0.82rem;">&copy; <?php echo date('Y'); ?> <strong>e-Capaian</strong>. All rights reserved.</p>
            <p class="mb-0" style="font-size: 0.78rem;">Powered by Clean Architecture & CodeIgniter 3.</p>
        </footer>
    </div>
